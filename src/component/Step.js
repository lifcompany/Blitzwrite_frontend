...
const Step = ({ end }) => {
    return (
        <ol className="flex items-center text-xs text-gray-900 font-medium sm:text-base">
            ...
            <li className={`flex w-full relative text-[#3BAAE2] ${end ? 'bg-[#3BAAE2] text-white' : 'border-dashed bg-gray-50'} border-2 rounded-full`}>
                <div className="block z-10">
                    <span className="w-10 h-10 border-2 border-[#3BAAE2] rounded-full flex font-bold justify-center items-center mx-auto mb-3 text-[16px]">{end ? '2' : ''}</span> 記事編集
                </div>
            </li>
        </ol>
    );
